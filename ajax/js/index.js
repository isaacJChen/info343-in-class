...
function fetchWeather(data) {
  ...
  let url = WEATHER_API.replace("{latitude}", latitude).replace("{longitude}", longitude);
  return fetch(url);
}
...
function renderWeather(data) {
  ...
  if(data.weather && data.weather[0]) {
    ...
    document.querySelector("#description").textContent = conditions.description;
...
console.log("line after fetch executes imediately");
